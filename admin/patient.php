...
<?php 
          ...
          $sql = "SELECT * FROM patient WHERE sex ='Others'";
          $result = mysqli_query($conn, $sql); 
          $rowcount = mysqli_num_rows($result);

          if ($result) {

            echo "['Others', ".$rowcount."],";
          }
...
<script src="sweetalert2.all.min.js"></script>     
  ...
  <?php 
        ...
        if(isset($_POST['save'])){
          ...
          $sql = "UPDATE patient SET payment = '$payment', plan = '$plan' WHERE id = $patient";
          $result = mysqli_query($conn, $sql);
...
<?php 
         if(empty($rows['image']))
         {
           echo '<img src="../uploads/default.png" width="180px" height="180px" style="margin-top: 20px; border-radius:100px; box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);">';
         } else{ ?>
          
          <img src="../uploads/<?php echo $rows['image']?>" width="30px" height="30px" width="180px" height="180px" style="margin-top: 20px; border-radius:100px; box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);"/>
    ...
    <?php     }
         ...
         ?>
                      ...
                      <h2 style="font-size: 22px;" class=" mt-4 mb-1"><?=$row['first']?> <?=$row['last']?></h2>
                      <p>@<?=$row['username']?></p>
                      ...
                      <p class="text-left ms-4 mb-0 mt-4" style="font-weight: 500;">Email Address:</p>
                       ...
                       <p class="text-left ms-4 mb-0 mt-4 " style="font-weight: 500;">Contact Number:</p>
                        ...
                        <p class="text-left ms-4 mb-0 mt-4" style="font-weight: 500;">Age:</p>
                            <h6 class="text-left ms-4 text-muted"> <?php $dateOfBirth = $row['birth'];
                  $today = date("Y-m-d");
...
<?php if(empty($row['hmo'])) { ?>
  ...
  <label class=" text-secondary form-label mt-2 mb-0" style="font-size: 16px; font-weight: 500">HMO is not applicable to this patient</label> <br>


      <?php } else { ?>

      <label class=" text-secondary form-label mt-2 mb-0" style="font-size: 16px; font-weight: 500">HMO is applicable to this patient</label> <br>

      <?php } ?>


        
        <p class="ms-4 mb-3 mt-5" style="font-weight: 500;">Accepted Payment:</p>
                        <select class="form-select mb-5" style="width: 250px; margin: auto"
        ...
        <button type="save" name="save" id="save" class="btn btn-info" >Accept Payment</button>
...
<div style="width:100%;overflow:auto; max-height:500px;">
             <table class="table table-hover" style="width:645px; margin: auto;">
                 
          <tbody style="font-weight: 400; color: #4C4C4C; border: none">

          <?php while ($rows = mysqli_fetch_assoc($result)) {
            if (empty($rows['patient'])) {
            ?>
