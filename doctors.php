...
$id = $_GET['sched'];
$qry = "SELECT * FROM doctors_schedule where doctor_id ='$id'";
...
?>    
          ...
          <?php 
              ...
              <tr>
                <td class="ps-4 pb-0 pt-0 text-start" style="border: none; margin-bottom: -1px">
                  <input class="form-check-input" style="margin-bottom: 10px; margin-left: 0px" type="checkbox" name="check[<?php echo $i ?>]" value="<?php echo isset($id[$days[$i]]) ? $id[$days[$i]] : '' ?>">>
          ...
          <button type="submit" name="saves" id="saves" class="btn btn-sm btn-primary col-sm-3 offset-md-3 mt-3">Save</button>
...
$view = $_GET['view'];
								$list = "SELECT * FROM doctors_schedule WHERE doctor_id='".$view."' order by id asc";
...
<form class="doctor border shadow p-4 rounded-20 ms-2" style="height: 700px; background-color: white;position:absolute; width: 400px; margin-top:-45px; margin-bottom:-15px;" method="POST" action="" enctype="multipart/form-data" id="manage-doctor">
<input type="hidden" name="id" value="<?php echo $id;?>">
              ...
              <div class="doctor">
                ...
                <input type="email" class="form-control" name="email" disabled value="<?=$_SESSION['cname']?>" style="width: 100%; margin-bottom: 6px;"> 
							...
							<div class="doctor">
								...
								<input required value="<?php echo $file?>" type="file" class="form-control" name="image" id="image" accept="image/jpg, image/jpeg" onchange="displayImg(this,$(this))">
							</div>
...
<form class="border shadow p-4" action="" method="POST" style="height: 630px; background-color: white;  margin-left: 420px; margin-top: -15px; width: 980px; max-height: 100%; " enctype="multipart/form-data">
    
        <label for="" class="mb-1" style="font-size: 22px; margin-top: -5px">List of Doctors</label>
        
        
        <div style="width:100%;overflow:auto; max-height:540px; margin: auto; z-index: 1;">       
        ...
        <?php 
								$list = "SELECT * FROM doctors_list WHERE cname='".$_SESSION['cname']."' order by id asc";
                $res = mysqli_query($conn, $list);
                ?> 
          <?php  while($row=$res->fetch_assoc()):
