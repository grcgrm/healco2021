...
if(isset($_POST['savec']))
{
  ...
  $lab= $_FILES["lab"]["name"];
  $tempname = $_FILES["lab"]["tmp_name"];
  $folder = "uploads/".$lab;
  
  $list = "INSERT INTO `cbc` (`clinic_id`, `patient_id`, `patient`, `clinic`, `lab`)
          VALUES('$clinic_id', '$patient_id','$patient','$clinic', '$lab')";
...
?>
 ...
 <?php if($cbc == false):
  ...
  ?>    
  <form class="bg-white border shadow p-4 text-left" id="form" action="" method="POST" style="border-radius: 20px; width: 410px; height: 658px; font-family: Poppins; margin-left: 40px; margin-top: -20px" enctype="multipart/form-data">
  ...
  <h5 class="card-title" style="margin-left: 120px; margin-top: -10px; font-size: 24px; color: #4a5454">CBC Blood Test</h5>
      ...
      <label class="mt-3" style="font-size: 15px;"> Upload Lab Results</label>
      <input type="file" class="form-control mt-3" name="lab" id="lab" value="<?php echo $lab_result?>">
...
<?php
              $id = $_SESSION['id'];
              $sql = "SELECT * FROM cbc WHERE clinic_id = '$id' ORDER BY ID ASC";
              $result = mysqli_query($conn, $sql); 
...
<div style="width:100%;overflow:auto; max-height:550px;">
          ...
          <tr style="border: none">
            ...
            <embed type="application/pdf" src="uploads/<?php echo $rows['lab']?>" width="300px" height="300px" />
...
<?php else: ?>
  ...
  if(isset($_POST['savechol']))
  {
    ...
    $file= $_FILES["chol"]["name"];
    $tempname = $_FILES["chol"]["tmp_name"];
	  $folder = "uploads/".$file;
        
    $list1 = "INSERT INTO cholesterol (`clinic_id`, `clinic`, `patient_id`, `patient`, `result_cho`)
        VALUES('$clinic_id', '$clinic','$patient_id','$patient', '$file')";
                $res = mysqli_query($conn, $list1);
...
<form class="bg-white border shadow p-4 text-left" id="cholesterol" action="" method="POST" style="border-radius: 20px; width: 410px; height: 680px; font-family: Poppins; margin-left: 40px; margin-top: -20px" enctype="multipart/form-data">
  ...
  <h5 class="card-title" style="margin-left: 120px; margin-top: -5px; font-size: 24px; color: #4a5454">Cholesterol Test</h5>
      ...
      <label class="mt-3" style="font-size: 15px;"> Upload Lab Results</label>
      <input required type="file" class="form-control mt-3" name="chol" id="image">
...
<div style="width:100%;overflow:auto; max-height:550px;">
          ...
          <tr style="border: none">
            ...
            <td>
            <embed type="application/pdf" src="uploads/<?php echo $rows['result_cho']?>" width="300px" height="300px" />
