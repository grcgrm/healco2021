...
if(isset($_POST['submit']))
{
    ...
    if ($password == $cpassword) {
		...
		if (!$result->num_rows > 0) {
			$sql = "INSERT INTO patient (name, username, email, password, phone, address, age, birth, sex, status, c_illness, m_problems, father, f_age, f_cause, mother, m_age, m_cause, disease, other, drug, reaction, medication, reason, clinic)
					VALUES ('$name','$username', '$email', '$password', '$phone', '$address', '$age', '$birth', '$sex', '$status', '$cdata', '$mdata' , '$father', '$f_age', '$f_cause', '$mother', '$m_age', '$m_cause', '$ddata', '$odata',  '$data', '$rdata', '$redata', '$medata', '$clinic')";
			$result = mysqli_query($conn, $sql);
